...
class TagController {
    ...
    def tagListForAutoCompletion() {
        List<Tag> tagList = Tag.findAllByNameIlike("%${params.term}%")
        def tags= tagList.collect {
    ...
    def list = {
        ...
        render([tagInstanceList: Tag.list(params)*.name, tagInstanceTotal: Tag.count()] as JSON)
    ...
    def save = {
        ...
        if (tagInstance.save(flush: true)) {
            flash.message = "${message(code: 'default.created.message', args: [message(code: 'tag.label', default: 'Tag'), tagInstance])}"
    ...
    def update = {
        ...
        if (tagInstance) {
            ...
            if (!tagInstance.hasErrors() && tagInstance.save(flush: true)) {
                flash.message = "${message(code: 'default.updated.message', args: [message(code: 'tag.label', default: 'Tag'), tagInstance])}"
                redirect(action: "show", id: tagInstance.id)
            }
            else {
                render(view: "edit", model: [tagInstance: tagInstance])
